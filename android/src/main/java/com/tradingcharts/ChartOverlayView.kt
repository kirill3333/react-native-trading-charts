...
internal class ChartOverlayView(context: Context) : View(context) {
  ...
  private fun prepare(config: ChartConfig) {
    ...
    percentFormat = NumberFormat.getNumberInstance(locale).apply {
      isGroupingUsed = false
      minimumFractionDigits = 2
      maximumFractionDigits = 2
    }
    volumeFormat = NumberFormat.getNumberInstance(locale).apply {
      isGroupingUsed = config.valueFormat.useGrouping
      minimumFractionDigits = 0
      maximumFractionDigits = 2
    }
    val axisDates = config.xAxisDateFormats
    axisDateFormats = listOf(
  ...
  private fun applyTextStyle(
    ...
    val bold = when (style.fontWeight) {
      "medium", "semibold", "bold" -> true
      "regular" -> false
      else -> defaultBold
    }
    val family = style.fontFamily?.let { Typeface.create(it, Typeface.NORMAL) } ?: Typeface.MONOSPACE
  ...
  private fun safeDateFormat(pattern: String, locale: String, zone: String, fallback: String): SimpleDateFormat {
    val result = try {
  ...
  private fun timeFormatIndex(frame: ChartSnapshot): Int {
    ...
    return when {
      span <= 5 * 60 * 1000.0 ||
        (frame.config.showSeconds && span <= 2 * 60 * 60 * 1000.0) -> 0
  ...
  override fun onDraw(canvas: Canvas) {
    ...
    if (frame.currentPriceVisible && config.showCurrentPriceLabel) {
      drawBadge(
        ...
        currentPriceTextPaint,
        config.currentPriceBorder,
        config,
      )
    }
  ...
  private fun drawBadge(
    ...
    val badgeY = y.coerceIn(halfHeight, max(halfHeight, snapshot!!.height - halfHeight))
    val width = min(config.yAxisWidth, textWidth + 12f * density)
    val x = if (config.yAxisOnRight) snapshot!!.plotRight else max(0f, snapshot!!.plotLeft - width)
    val rect = RectF(x, badgeY - halfHeight, x + width, badgeY + halfHeight)
    drawBackground(canvas, rect, backgroundColor, border)
    canvas.drawText(text, rect.left + 6f * density, centeredBaseline(rect.centerY(), textPaint), textPaint)
  }
  ...
  private fun drawTimeBadge(canvas: Canvas, text: String, frame: ChartSnapshot) {
    ...
    drawBackground(
      canvas, rect, frame.config.crosshairTimeBackgroundColor, frame.config.crosshairTimeBorder,
    )
    canvas.drawText(
      text, rect.left + 6f * density, centeredBaseline(rect.centerY(), crosshairTimeTextPaint),
  ...
  private fun drawExtremum(
    ...
    val lineEndX = (extremum.x + direction * 20f * density).coerceIn(frame.plotLeft, frame.plotRight)
    canvas.drawLine(extremum.x, extremum.y, lineEndX, extremum.y, extremumLinePaint)
    val unclampedX = if (extremum.labelOnRight) lineEndX + 4f * density
    ...
    val baseline = centeredBaseline(extremum.y, extremaTextPaint).coerceIn(
      frame.plotTop - extremaTextPaint.ascent(),
      max(frame.plotTop - extremaTextPaint.ascent(), frame.plotBottom - extremaTextPaint.descent()),
    )
    canvas.drawRoundRect(
      x - 2f * density,
  ...
  private fun drawTooltip(canvas: Canvas, frame: ChartSnapshot) {
    ...
    val rect = RectF(
      left, frame.plotTop + 8f * density, left + layout.width,
    ...
    drawBackground(canvas, rect, Color.argb(alpha, Color.red(background), Color.green(background), Color.blue(background)), config.tooltipBorder)
  ...
  private fun prepareTooltipLayout(frame: ChartSnapshot): TooltipLayout {
    ...
    tooltipLayout?.let { cached ->
      if (
        tooltipConfig === frame.config && tooltipCandle?.contentEquals(c) == true &&
    ...
    val rows = listOf(
      TooltipRow(labels.open, formatValue(c[1], tooltipValueFormat), config.tooltipValueTextStyle.color),
      TooltipRow(labels.close, formatValue(c[4], tooltipValueFormat), config.tooltipValueTextStyle.color),
      TooltipRow(labels.high, formatValue(c[2], tooltipValueFormat), config.tooltipValueTextStyle.color),
      TooltipRow(labels.low, formatValue(c[3], tooltipValueFormat), config.tooltipValueTextStyle.color),
      TooltipRow(labels.amplitude, formatPercent(frame.selectedAmplitudePercent, frame.selectedPercentagesValid), config.tooltipValueTextStyle.color),
      TooltipRow(labels.changePercent, formatPercent(frame.selectedChangePercent, frame.selectedPercentagesValid), changeColor),
      TooltipRow(labels.change, formatValue(frame.selectedChange, tooltipValueFormat), changeColor),
      TooltipRow(labels.volume, formatVolume(c[5]), config.tooltipValueTextStyle.color),
    )
    val header = tooltipHeaderDateFormat.format(Date(c[0].toLong()))
    ...
    val contentWidth = max(tooltipHeaderPaint.measureText(header), labelWidth + 12f * density + valueWidth)
    val headerHeight = max(17f * density, tooltipHeaderPaint.descent() - tooltipHeaderPaint.ascent())
    val rowHeight = max(
      17f * density,
      max(
    ...
    return TooltipLayout(
      header, rows, contentWidth + 20f * density,
      18f * density + headerHeight + rows.size * rowHeight,
      valueXOffset, headerHeight, rowHeight,
    ).also {
      tooltipLayout = it
  ...
  private fun colorFromFloats(value: FloatArray) = Color.argb(
    (value[3] * 255).toInt(), (value[0] * 255).toInt(),
